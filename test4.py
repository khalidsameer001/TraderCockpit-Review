...
st.markdown(
    f"<h3>{symbol} | <span style='color:{pnl_color}'>P/L: ${pnl}</span></h3>",
    unsafe_allow_html=True
)
...
with right:
    ...
    premarket_gap = st.text_input(
        "Premarket Gap %",
        row.get("Premarket Gap %", "")
    )

    day_range = st.text_input(
        "Day Range %",
        row.get("Day Range %", "")
    )
    ...
    catalyst_type = st.text_input(
        "Catalyst Type",
        row.get("Catalyst Type", "")
    )
